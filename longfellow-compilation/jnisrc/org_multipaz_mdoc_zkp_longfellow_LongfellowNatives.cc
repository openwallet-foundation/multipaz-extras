...
std::vector<RequestedAttribute> parse_statement(JNIEnv *env,
    ...
    for (jsize i = 0; i < length; ++i) {
        ...
        // Get the namespaces string and namespace size
        jfieldID namespaceField =
                env->GetFieldID(statementClass, "namespace", "Ljava/lang/String;");
        jstring namespaceString = (jstring) env->GetObjectField(statement, namespaceField);
        const char *namespaceChars = env->GetStringUTFChars(namespaceString, nullptr);
        jsize namespaceLength = env->GetStringUTFLength(namespaceString);
        ...
        std::vector<uint8_t> valueByteArray(valueBytes, valueBytes + valueLength);
        ...
        memcpy(newRa.namespace_id, namespaceChars, std::min((size_t)namespaceLength, sizeof(newRa.namespace_id)));
        newRa.namespace_len = std::min((size_t)namespaceLength, sizeof(newRa.namespace_id));

        memcpy(newRa.cbor_value, valueByteArray.data(),
               std::min(valueByteArray.size(), sizeof(newRa.cbor_value)));
        newRa.cbor_value_len = std::min(valueByteArray.size(), sizeof(newRa.cbor_value));

        requested_attributes.push_back(newRa);

        //env->ReleaseStringUTFChars(namespaceString, namespaceChars);
...
ZkSpecStruct* getZkSpec(JNIEnv* env, jobject jzk_spec) {
    ...
    jfieldID blockEncHashField = env->GetFieldID(zkSpecClass, "blockEncHash", "J");
    jfieldID blockEncSigField = env->GetFieldID(zkSpecClass, "blockEncSig", "J");
    ...
    jlong blockEncHash = env->GetLongField(jzk_spec, blockEncHashField);
    jlong blockEncSig = env->GetLongField(jzk_spec, blockEncSigField);
    ...
    ZkSpecStruct* result = new(memory) ZkSpecStruct{
            ...
            static_cast<size_t>(blockEncHash),
            static_cast<size_t>(blockEncSig)
    };
...
Java_org_multipaz_mdoc_zkp_longfellow_LongfellowNatives_generateCircuitNative(JNIEnv *env,
									      jclass clazz, jobject jzk_spec
									      ){
  ...
  if (zk_spec == nullptr) {
    fprintf(stderr, "Cannot parse ZkSpec.");
    return nullptr;
  }
  ...
  if (result != CIRCUIT_GENERATION_SUCCESS) {
    fprintf(stderr, "Circuit generation failed with error code: %d", result);
    return nullptr;
  }
