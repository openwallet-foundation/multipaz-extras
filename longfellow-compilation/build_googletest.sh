...
# build_googletest.sh
#
# This is a universal script to build the GoogleTest (gtest) library for
# iOS, Android, macOS, and Linux. It creates static libraries and, where
# appropriate, provides commands to create XCFrameworks.
#
# Usage:
#   ./build_googletest.sh <platform>
#
# Supported Platforms:
#   ios
#   android
...
GTEST_VERSION="v1.14.0" # Use a specific git tag from the googletest repository
...
SOURCE_DIR="${SCRIPT_DIR}/source/googletest"
OUTPUT_DIR="${SCRIPT_DIR}/output/googletest/${TARGET_PLATFORM}"
BUILD_DIR="${SCRIPT_DIR}/build/googletest/${TARGET_PLATFORM}"
...
clone_googletest() {
    if [ ! -d "${SOURCE_DIR}" ]; then
        log "Cloning GoogleTest source code (version ${GTEST_VERSION})..."
        git clone --depth 1 --branch ${GTEST_VERSION} https://github.com/google/googletest.git "${SOURCE_DIR}"
    else
        log "GoogleTest source directory found. Checking out version ${GTEST_VERSION}..."
        cd "${SOURCE_DIR}"
        git fetch --all --tags
...
##
build_ios() {
    log "Starting GoogleTest iOS build..."
    ...
    build_ios_arch() {
        ...
        log "Building GoogleTest for iOS: ${PLATFORM} (${ARCH})"
    ...
    # Create fat libraries for each gtest/gmock component
    #lipo -create \
    #    "${BUILD_DIR}/install/arm64-iphoneos/lib/libgtest.a" \
    #    "${BUILD_DIR}/install/x86_64-iphonesimulator/lib/libgtest.a" \
    #    "${BUILD_DIR}/install/arm64-iphonesimulator/lib/libgtest.a" \
    #    -output "${OUTPUT_DIR}/lib/libgtest.a"
    #lipo -create \
    #    "${BUILD_DIR}/install/arm64-iphoneos/lib/libgtest_main.a" \
    #    "${BUILD_DIR}/install/x86_64-iphonesimulator/lib/libgtest_main.a" \
    #    "${BUILD_DIR}/install/arm64-iphonesimulator/lib/libgtest_main.a" \
    #    -output "${OUTPUT_DIR}/lib/libgtest_main.a"
    #lipo -create \
    #    "${BUILD_DIR}/install/arm64-iphoneos/lib/libgmock.a" \
    #    "${BUILD_DIR}/install/x86_64-iphonesimulator/lib/libgmock.a" \
    #    "${BUILD_DIR}/install/arm64-iphonesimulator/lib/libgmock.a" \
    #    -output "${OUTPUT_DIR}/lib/libgmock.a"
    #lipo -create \
    #    "${BUILD_DIR}/install/arm64-iphoneos/lib/libgmock_main.a" \
    #    "${BUILD_DIR}/install/x86_64-iphonesimulator/lib/libgmock_main.a" \
    #    "${BUILD_DIR}/install/arm64-iphonesimulator/lib/libgmock_main.a" \
    #    -output "${OUTPUT_DIR}/lib/libgmock_main.a"
}
...
##
build_android() {
    log "Starting GoogleTest Android build..."
    ...
    build_android_arch() {
        ...
        log "Building GoogleTest for Android: ${ARCH_NAME}"
        mkdir -p "${CMAKE_BUILD_DIR}"
...
##
build_macos() {
    log "Starting GoogleTest macOS build..."
    ...
    build_macos_arch() {
        ...
        log "Building GoogleTest for macOS: ${ARCH}"
    ...
    #lipo -create "${BUILD_DIR}/install/x86_64/lib/libgtest.a" "${BUILD_DIR}/install/arm64/lib/libgtest.a" -output "${OUTPUT_DIR}/lib/libgtest.a"
    #lipo -create "${BUILD_DIR}/install/x86_64/lib/libgtest_main.a" "${BUILD_DIR}/install/arm64/lib/libgtest_main.a" -output "${OUTPUT_DIR}/lib/libgtest_main.a"
    #lipo -create "${BUILD_DIR}/install/x86_64/lib/libgmock.a" "${BUILD_DIR}/install/arm64/lib/libgmock.a" -output "${OUTPUT_DIR}/lib/libgmock.a"
    #lipo -create "${BUILD_DIR}/install/x86_64/lib/libgmock_main.a" "${BUILD_DIR}/install/arm64/lib/libgmock_main.a" -output "${OUTPUT_DIR}/lib/libgmock_main.a"
}
...
##
build_linux() {
    log "Starting GoogleTest Linux build..."
    ...
    build_linux_arch() {
        ...
        log "Building GoogleTest for Linux: ${ARCH}"
        mkdir -p "${CMAKE_BUILD_DIR}"
